...
class Update extends AbstractService {
	...
	protected function get_remote_latest() {
		if ( $this->cache->is_throttled( 'update_check' ) ) {
			return new WP_Error( 'too_many_requests', __( 'Too many requests. Slow down.', 'foxelabs-freemius' ) );
		}

		$activation = $this->activations->get( $this->plugin->get_id() );
		if ( ! $activation->is_active() ) {
			return new WP_Error( 'license_not_active', __( 'No valid license is active.', 'foxelabs-freemius' ) );
		}
