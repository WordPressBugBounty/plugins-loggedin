 ...
 * {@see \FoxeLabs\Freemius\Storage\ActivationRepository}, and
 * computes the host site UID through
 * {@see \FoxeLabs\Freemius\Support\SiteIdentity}.
 *
...
use FoxeLabs\Freemius\Data\Activation;
...
class License extends AbstractService {
	...
	public function activate( string $key ) {
		if ( '' === $key ) {
			return new WP_Error( 'empty_activation_key', __( 'License key is empty.', 'foxelabs-freemius' ) );
		}

		// Only premium plugins require a license.
		if ( ! $this->plugin->is_premium() ) {
			return new WP_Error( 'not_premium', __( 'Not a premium plugin.', 'foxelabs-freemius' ) );
		}
		...
		if ( isset( $response['install_id'] ) ) {
			 ...
			 */
			do_action( 'foxelabs_freemius_license_activated', $activation->to_array(), $success );

			return $success;
		}

		return new WP_Error( 'unknown_error', __( 'Unknown error.', 'foxelabs-freemius' ) );
	}
	...
	public function deactivate() {
		...
		if ( ! $this->can_deactivate( $activation ) ) {
			return new WP_Error( 'invalid_activation_data', __( 'Invalid activation data.', 'foxelabs-freemius' ) );
		}
		...
		if ( isset( $response['id'] ) ) {
			 ...
			 * @param bool  $success    Whether the option update succeeded.
			 */
			do_action( 'foxelabs_freemius_license_deactivated', $activation->to_array(), $success );
