 ...
 *
 * Default {@see \FoxeLabs\Freemius\Contracts\CacheInterface} for the
 * library. Each instance is scoped to one host plugin, so cache
 * collisions between multiple Foxe Labs plugins on the same site are
 * impossible — every key is prefixed with `foxelabs_freemius_{id}_`.
 *
...
namespace FoxeLabs\Freemius\Storage;

// If this file is called directly, abort.
defined( 'WPINC' ) || die;

use FoxeLabs\Freemius\Contracts\CacheInterface;
...
class TransientCache implements CacheInterface {
	...
	private function build_key( string $key ): string {
		return 'foxelabs_freemius_' . $this->plugin->get_id() . '_' . $key;
	}
