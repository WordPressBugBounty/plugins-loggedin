 ...
 *
 * The bundled `foxelabs_reviews_notice_message` filter is preserved
...
namespace FoxeLabs\Reviews\Rendering;
...
class MessageBuilder {
	 ...
	 * passed through the `foxelabs_reviews_notice_message` filter so
	...
	public function build( Config $config ): string {
		 ...
		 * @since 1.0.2
		 */
		return (string) apply_filters( 'foxelabs_reviews_notice_message', $message, $config->days() );
	}
	 ...
	 * consumer locale, and the `foxelabs_reviews_notice_message`
